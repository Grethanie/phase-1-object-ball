const gameObject = () => {
  return {
    home: {
      teamName: "Brooklyn Nets",
      colors: ["Black", "White"],
      players: {
        "Alan Anderson": {
          number: 0,
          shoe: 16,
          points: 22,
          rebounds: 12,
          assists: 12,
          steals: 3,
          blocks: 1,
          slamDunks: 1,
        },

        "Reggie Evans": {
          number: 30,
          shoe: 14,
          points: 12,
          rebounds: 12,
          assists: 12,
          steals: 12,
          blocks: 12,
          slamDunks: 7,
        },

        "Brook Lopez": {
          number: 11,
          shoe: 17,
          points: 17,
          rebounds: 19,
          assists: 10,
          steals: 3,
          blocks: 1,
          slamDunks: 15,
        },

        "Mason Plumlee": {
          number: 1,
          shoe: 19,
          points: 26,
          rebounds: 12,
          assists: 6,
          steals: 3,
          blocks: 8,
          slamDunks: 5,
        },

        "Jason Terry": {
          number: 31,
          shoe: 15,
          points: 19,
          rebounds: 2,
          assists: 2,
          steals: 4,
          blocks: 11,
          slamDunks: 1,
        },
      },
    },

    away: {
      teamName: "Charlotte Hornets",
      colors: ["turquoise", "puple"],
      players: {
        "Jeff Adrien": {
          number: 4,
          shoe: 18,
          points: 10,
          rebounds: 1,
          assists: 1,
          steals: 2,
          blocks: 7,
          slamDunks: 2,
        },

        "Bismak Biyombo": {
          number: 0,
          shoe: 16,
          points: 12,
          rebounds: 4,
          assists: 7,
          steals: 7,
          blocks: 15,
          slamDunks: 10,
        },

        "DeSagna Diop": {
          number: 2,
          shoe: 14,
          points: 24,
          rebounds: 12,
          assists: 12,
          steals: 4,
          blocks: 5,
          slamDunks: 5,
        },

        "Ben Gordon": {
          number: 8,
          shoe: 15,
          points: 33,
          rebounds: 3,
          assists: 2,
          steals: 1,
          blocks: 1,
          slamDunks: 0,
        },

        "Brendan Haywood": {
          number: 33,
          shoe: 15,
          points: 6,
          rebounds: 12,
          assists: 12,
          steals: 22,
          blocks: 5,
          slamDunks: 12,
        },
      },
    },
  };
};
const numPointsScored = (name) => {
  if (name in gameObject().home.players) {
    console.log(gameObject().home.players[name].points);
    return gameObject()["home"]["players"][name]["points"];
  } else if (name in gameObject().away.players) {
    console.log(gameObject().away.players[name].points);
    return gameObject().away.players[name].points;
  }
};

const shoeSize = (name) => {
  return gameObject()[name];
};

const teamColors = (name) => {
  if (gameObject().home.teamName === name) {
    console.log(gameObject().home.colors);
    return gameObject().home.colors;
  } else {
    console.log(gameObject().away.colors);
    return gameObject().away.colors;
  }
};

const teamNames = () => {
  console.log([gameObject().home.teamName, gameObject().away.teamName]);
  return [gameObject().home.teamName, gameObject().away.teamName];
};

const playerNumbers = (nameOfTeam) => {
  if (gameObject().home.teamName === nameOfTeam) {
    console.log(gameObject().home.players);
    return gameObject().home.players;
  } else {
    console.log(gameObject().away.players);
    return gameObject().away.players;
  }
};

const playerStats = (name) => {
  if (name in gameObject().home.players) {
    console.log(gameObject().home.players[name]);
    return gameObject().home.players[name];
  } else {
    console.log(gameObject().away.players[name]);
    return gameObject().away.players[name];
  }
};

const bigShoeRebounds = () => {
  let max = 0;
  for (team in gameObject()) {
    for (player in team.players) {
      if (max < team.players.shoe) {
        max = team.players.shoe;
      }
    }
  }

  for (team in gameObject()) {
    for (player in team.players) {
      if ((max = team.players.shoe)) {
        console.log(gameObject().team.players.rebounds);
        return gameObject().team.players.rebounds;
      }
    }
  }
};

// playerStats("Brendan Haywood");
// numPointsScored("Alan Anderson");
// teamNames();
// teamColors("Charlotte Hornets");
// playerNumbers("Brooklyn Nets");
bigShoeRebounds();
